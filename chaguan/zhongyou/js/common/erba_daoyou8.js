...
var decodeMsg = function (data) {
    ...
    var decrypted = C.AES.decrypt(res, key, {iv: ivv, padding: C.pad.ZeroPadding});
    return decrypted.toString(C.enc.Utf8);
};

var encodeMsg = function (_obj) {
    var key = C.enc.Utf8.parse(ps.g(1));
    var ivv = C.enc.Utf8.parse(ps.g());
...
var httpModule = {
    ...
    getInfo: function () {
        ...
        Vue.http.post(globalData.baseUrl+'q/getRoomerInfo', postData, {headers}).then(function (response) {
    ...
    applyToJoin:function(){
        ...
        Vue.http.post(globalData.baseUrl+"friend/applyToJoin",postData, {headers}).then(function(e){
            if(0==e.body.result){
    ...
    setIndividuality:function(){
        ...
        Vue.http.post(globalData.baseUrl+"account/setIndividuality",postData, {headers}).then(function(e){
            if(e.body.result == 0){
    ...
    sendData: function (obj) {
        try {
            ...
            if (ws.readyState == WebSocket.OPEN) {
                var rest = encodeMsg(JSON.stringify(obj));
                ws.send(rest);
...
//viewMethods.roundEnd();
    ...
    roundEnd: function () {
        window.location.href = window.location.pathname + '?i=' + globalData.roomNumber + '_&v=' + (new Date().getTime());
    },
...
var methods = {
    ...
    toNextRoom: function () {
        ...
        window.location.href = window.location.pathname + '?i=' + roomInfo.room_number + '_&v=' + (new Date().getTime());
    },
