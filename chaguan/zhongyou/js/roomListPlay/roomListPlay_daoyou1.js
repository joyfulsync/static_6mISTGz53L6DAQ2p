var httpModule = {
    loadMoreScoreList: function (type) {
        var data = {
        ...
        Vue.http.post(request_url + 'game/roomStatusList', data,{emulateJSON:true}).then(function(response) {
            ...
            if (bodyData.result == 0) {
                ...
                for (var i = 0; i < bodyData.data.length;i++) {
                    ...
                    appData.gameScoreList.push({"number":item.room_number,"time":item.start_time2,"score":item.score,"game_type":item.game_type});
                }
            } else {
...
function loadMoreScoreList(type) {
    appData.page = parseInt(appData.page) + 1;
    httpModule.loadMoreScoreList(type);
    $('#moretext').show();
    $('#moretext').text('加载中...');
};
...
for (var i = 0; i < globalData.gameList.length; i++) {
    appData.gameItems.push({
        "avatar":globalData.cfileUrl+"files/images/gameicon/"+globalData.gameList[i].game_icon,
...
var methods = {
    ...
    clickGame:function (item) {
        try {
            ...
            var selectGame = 'game' + item.game_type;
            appData.selectedGame = item.game_type;
            appData.gameScoreList = [];
            appData.page = 1;
            appData.sumPage = 1;
            httpModule.loadMoreScoreList(item.game_type);
  ...
  clickScoreItem: function (room_number,game_type) {
		game_type = String(game_type)
        // var url = request_url + 'game/queryCard?type=' + item.game_type + '&num='+item.number;
        var url = ""
        switch (game_type) {
            case '1':
        ...
        window.location.href = url + "?num="+room_number+"&type="+game_type;
    },
};
...
var vueLife = {
    vmCreated: function () {
        ...
        $("#loading").hide();
        $(".main").show();
        appData.selectedGame=1;
        httpModule.loadMoreScoreList(appData.selectedGame);
    },
    vmUpdated: function () {
