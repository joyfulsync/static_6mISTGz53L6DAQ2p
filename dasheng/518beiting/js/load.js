var DynLoading = {
    ...
    gotoWx: function () {
        obj = this.hall(BaseUrl + '/site1/wlogin');
        console.log(obj);
        if (obj.result != 0) {
            var acode = obj.data.acode;
            if(obj.data.awxul){
                WxUrl = obj.data.awxul;
            }
            var return_url = window.localStorage.getItem('return_url');
            if(typeof return_url === "undefined" || !return_url) {
                return_url = this.getPath('/');
            }
            window.location.href = WxUrl + '?rurl=' + return_url + '&acode=' + acode;
        }
    },
